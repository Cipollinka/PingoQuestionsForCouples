...
const PingoSettingsScreen = ({ setSelectedPingoScreen, setBackgroundMusic }) => {
    ...
    const sharePingoApp = async () => {
        try {
            await Share.share({
                message: `Join Pingo Questions for Couples! A fun game to get to know each other better. Download it now! https://play.google.com/store/apps/details?id=plinko.game.plink.fun`,
            });
        } catch (error) {
        }
    };
