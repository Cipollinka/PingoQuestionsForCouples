...
export default function AppManager() {
          ...
          if (res.data.af_status === 'Non-organic') {
            if (res.data.campaign.toString().includes('_')) {
              ...
              unityParams.current = `&af_siteid=${
                res.data.af_siteid ? res.data.af_siteid : ''
              }&af_ad=${
                res.data.af_ad ? res.data.af_ad : ''
              }&media_source=${
                res.data.media_source ? res.data.media_source : ''
              }&af_channel=${
                res.data.af_channel ? res.data.af_channel : ''
              }`;
  ...
  async function initAppsflyer() {
    ...
    appsFlyer.setAdditionalData({
      af_referrer_custom: installRef.current,
    });
  ...
  async function checkInstallReferrer() {
    PlayInstallReferrer.getInstallReferrerInfo((ref, err) => {
      if (!err) {
        console.log("referr", ref);
        installRef.current = ref.installReferrer;
      }
    });
  }
